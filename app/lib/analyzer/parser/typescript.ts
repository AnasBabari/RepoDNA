...
export class TypeScriptSyntaxParser implements SyntaxParser {
  ...
  async parse(input: ParseInput): Promise<ParsedSyntax> {
    ...
    try {
      ...
      if (!root) return { facts: failedFacts('typescript') };
      ...
      return { facts: { language: 'typescript', symbols: state.symbols, imports: state.imports, calls: state.calls, hasMainGuard: false, parse: { success: true, hasErrors, errorNodes: state.errorNodes }, quality: hasErrors ? 'partial' : 'complete' } };
    } catch { return { facts: failedFacts('typescript') }; } finally { if (tree) try { tree.delete(); } catch {} }
  }
}
...
function walk(node: SyntaxNode, state: WalkState, stack: Frame[], decorators: SyntaxDecorator[], depth: number, source: string): void {
  ...
  switch (node.type) {
    ...
    case 'export_statement': {
      for (const child of node.namedChildren) walk(child, state, stack, decorators, depth + 1, source);
      return;
    }
...
function rangeOf(node: SyntaxNode): SourceRange {
  return { startLine: node.startPosition.row + 1, endLine: node.endPosition.row + 1, startColumn: node.startPosition.column, endColumn: node.endPosition.column };
}
function isExported(node: SyntaxNode): boolean {
  const parent = (node as unknown as { parent?: SyntaxNode }).parent;
  if (parent && parent.type === 'export_statement') return true;
  return node.text.slice(0, 100).startsWith('export');
}
function extractHeritage(node: SyntaxNode): string[] {
  ...
  if (!heritage) {
    ...
    if (!alt) return [];
    return alt.text.split(',').map((s) => s.replace(/extends|implements/g, '').trim()).filter(Boolean);
  }
...
function extractMethodDecorators(node: SyntaxNode): SyntaxDecorator[] {
  ...
  for (const child of node.children) {
    if (child.type === 'decorator') {
      ...
      decorators.push({ name, argumentsText: argsMatch ? argsMatch[1] : null, range: rangeOf(child as unknown as SyntaxNode) });
    }
  }
  return decorators;
}
